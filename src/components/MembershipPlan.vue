...
const shouldShowSpecialOffer = (plan) => {
  ...
  // 季付且价格为88元时显示
  if (paymentType !== 'annual' && price === 88) {
    return true;
  }
...
const formatBenefitText = (text, monthlyPoints) => {
  ...
  // 处理可能的占位符格式：{monthly_smart_points}、{0}、{points} 等
  let processedText = text;
  if (monthlyPoints !== undefined && monthlyPoints !== null) {
    const pointsStr = monthlyPoints.toString();
    
    // 替换各种可能的占位符格式
    processedText = processedText
      .replace(/\{monthly_smart_points\}/gi, pointsStr)
      .replace(/\{0\}/g, pointsStr)
      .replace(/\{points\}/gi, pointsStr)
      .replace(/\{月额度\}/g, pointsStr)
      .replace(/\{\}/g, pointsStr);
    
    // 如果文本中包含"每月"和"条数据额度"，但中间没有数字，则插入数字
    // 例如："每月 条数据额度" -> "每月5000条数据额度"
    if (processedText.includes('每月') && processedText.includes('条数据额度')) {
      const monthlyMatch = processedText.match(/每月\s*条数据额度/);
      if (monthlyMatch) {
        processedText = processedText.replace(/每月\s*条数据额度/, `每月${pointsStr}条数据额度`);
      }
    }
  }
  
  // 如果文本中包含数字，高亮显示（使用更明确的样式类）
  const result = processedText.replace(/(\d+)/g, '<span class="highlight-red" style="color: #ffffff !important; background-color: #ff4d4f !important; font-weight: 700 !important; padding: 3px 8px !important; border-radius: 4px !important; display: inline-block !important;">$1</span>');
...
.feature-text :deep(.highlight-red),
.feature-text .highlight-red,
.highlight-red {
  ...
  box-shadow: 0 1px 3px rgba(255, 77, 79, 0.3) !important;
}
